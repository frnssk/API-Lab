(function() {
  ...
  document.addEventListener("keyup", function(event) {
    if (event.keyCode == 65) {
      blob.isLeftPressed = false;
    } else if (event.keyCode == 68) {
      blob.isRightPressed = false;
    }
  });
  ...
  //For smooth movement
  function blobMovementHandler() {
    blob.posX += blob.velocityX;

    if (blob.isRightPressed === true && blob.velocityX < blob.maxVelocity) {
      blob.velocityX += blob.step;
    } else if (
      blob.isLeftPressed === true &&
      blob.velocityX > blob.minVelocity
    ) {
      blob.velocityX -= blob.step;
